...
double CRF_Model::FunctionGradient(const vector<double> & x, vector<double> & grad)
{
			...
			const int max_num, const double min_prob)
{
  ...
  // WARNING This fp is used as a kind of testing - assertion
  //const double fp = forward_prob(seq.vs.size());
  //assert(abs(fp - 1) < 0.01);
...
void CRF_Model::add_sample_empirical_expectation(const Sequence & seq, vector<double> & vee)
{
  for (size_t i = 0; i < seq.vs.size(); i++) 
  {
...
{
    // WARNING fp i used as some kind of assertion - testig !
		 ...
		 //		 const double Nsigma2, const double widthfactor)
{
  ...
  for (unsigned int i = 0; i < _vee.size(); i++) _vee[i] /= _vs.size();
...
{
  _feature2mef.clear();
  for (unsigned int i = 0; i < _featurename_bag.Size(); i++) 
  {
    vector<int> vi;
    for (int k = 0; k < _num_classes; k++) {
...
CRF_Model::perform_AveragedPerceptron()
{
  ...
  while(iter < 5) {
	...
	for (vector<int>::const_iterator j = s.positive_features.begin(); j != s.positive_features.end(); j++){
      ...
      // edge
      for (unsigned int i = 0; i < seq.vs.size() - 1; i++) 
      {
...
{
  vector<double> v1 = v;
  for (unsigned int i = 0; i < v1.size(); i++) v1[i] = abs(v1[i]);
...
int CRF_Model::perform_StochasticGradientDescent()
{
  ...
  for (unsigned int i = 0; i < ri.size(); i++) ri[i] = i;
